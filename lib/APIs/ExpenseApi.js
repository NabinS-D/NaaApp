...
export const addExpenses = async (newExpense, userId, customCreatedAt = null) => {
  ...
  const response = await databases.createDocument(
    ...
    ID.unique(),
    {
      ...
      createdAt: customCreatedAt || new Date().toISOString(), // Use custom date if provided
    }
  );
  return response;
};
...
export const bulkImportExpenses = async (expensesData, userId) => {
  ...
  for (let i = 0; i < expensesData.length; i += batchSize) {
    ...
    try {
      // Create promises for current batch
      const batchPromises = batch.map(async (expenseData, index) => {
        try {
          await addExpenses(
            {
              ...
              receiptImage: null // No receipt images for bulk import
            },
            userId,
            expenseData.createdAt // Use the original date from import data
          );
          results.success++;
        } catch (error) {
          results.failed++;
      ...
      // Add delay between batches (except for the last batch)
      if (i + batchSize < expensesData.length) {
        await delay(delayBetweenBatches);
      }
      ...
      batch.forEach((_, index) => {
        results.failed++;
        results.errors.push(`Row ${i + index + 1}: Batch failed - ${error.message}`);
      });
    }
  }
